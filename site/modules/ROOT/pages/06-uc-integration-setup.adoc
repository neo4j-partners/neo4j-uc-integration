...
The SQL-to-Cypher translator now supports GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, and their full combinations:

[cols="2,1,2"]
|===
|Pattern |Status |Example

|GROUP BY (implicit grouping)
|PASS
|`SELECT name, count(*) FROM People GROUP BY name`

|GROUP BY (explicit WITH-clause)
|PASS
|`SELECT sum(age) FROM People GROUP BY name`

|HAVING (simple and compound)
|PASS
|`SELECT name, count(*) AS cnt FROM People GROUP BY name HAVING cnt > 5`
...
|PASS
|`SELECT name, count(*) AS cnt FROM People GROUP BY name ORDER BY cnt`

|DISTINCT with GROUP BY/HAVING
|PASS
|`SELECT DISTINCT name FROM People GROUP BY name HAVING count(*) > 5`

|LIMIT and OFFSET
|PASS
|`... GROUP BY name ORDER BY name LIMIT 10 OFFSET 5`

|WHERE + GROUP BY combinations
|PASS
|`SELECT count(*) FROM People WHERE age > 18 GROUP BY name`
...
|PASS
|`SELECT name FROM People GROUP BY name HAVING count(DISTINCT age) > 3`
...
NOTE: The examples above cover GROUP BY, HAVING, ORDER BY, LIMIT/OFFSET, DISTINCT, and their combinations with WHERE and JOIN. *Coming soon:* non-aggregate SELECT (`SELECT col1, col2 FROM Label`) and relationship property aggregation (aggregating over properties stored on Neo4j relationships rather than node properties).
...
The practical implication: UC JDBC now works well for aggregate analytics with GROUP BY, HAVING, ORDER BY, LIMIT, and DISTINCT, in addition to simple aggregates with WHERE filters and JOINs. For row-level data access or relationship property aggregation, use the Neo4j Spark Connector instead.
...
|GROUP BY
|PASS
|Implicit and explicit WITH-clause grouping
...
|HAVING
|PASS
|Simple, compound, and mixed aggregates

|LIMIT / OFFSET
|PASS
|Correct attachment to final RETURN

|DISTINCT + GROUP BY
|PASS
|Correct RETURN DISTINCT placement
...
14 of 14 supported patterns passed. 1 expected failure confirmed. Total execution time across all tests was approximately 194 seconds, with most of the time spent on connection initialization rather than query execution.
...
The federated queries in this guide use UC JDBC for aggregate counts (including GROUP BY, HAVING, ORDER BY) and the Spark Connector for row-level data, combining both in a single analysis when needed. Query 5 (Fleet Health Dashboard) demonstrates this hybrid approach.

== References

* https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[Neo4j UC JDBC Setup Guide] - Full configuration reference
