...
image::03-sensor-maintenance-flow.png[Sensor Health + Maintenance Correlation Flow,700]
...
The Neo4j Federated JDBC UC Connection's `remote_query()` wraps queries in subqueries for schema inference, which prevents GROUP BY. The Spark Connector loads row-level data that Spark SQL can freely aggregate and JOIN, which is essential for per-aircraft grouping.
