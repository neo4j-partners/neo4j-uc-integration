...
[.lead]
Genie and AI/BI dashboards both require data sources that Databricks can govern directly. Today, Neo4j data must be materialized into Delta before Genie can query it or a dashboard can connect to it — which means a copy, a pipeline to maintain, and a freshness problem. As an official federated data source, Neo4j would be queryable directly from both. This page demonstrates both capabilities using Lakebase.

== Scenario 3: Genie

Genie is where official federation makes the biggest visible difference, because today Genie requires Delta tables. To ask a natural language question about Neo4j or PostgreSQL data, you first have to copy that data into Delta, and then Genie queries the copy. That copy goes stale the moment the source changes.
...
For Neo4j, the contrast is direct: either you can point Genie at your database or you cannot. With an official integration, you can.
