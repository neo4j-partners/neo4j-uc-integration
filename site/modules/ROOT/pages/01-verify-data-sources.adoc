...
:description: Verify Delta lakehouse tables and Neo4j Federated JDBC UC Connection before running federated queries

Confirm both the Delta lakehouse tables and the Neo4j Federated JDBC UC Connection are accessible before running federated queries.

**Federation Method:** `remote_query()` (Neo4j Federated JDBC UC Connection) + Delta SQL

== Data Flow

image::01-verify-data-flow.png[Verify Data Sources Flow,700]
...
== Verify Neo4j Federated JDBC UC Connection
...
* The Neo4j Federated JDBC UC Connection (`remote_query()`) can execute SQL queries against Neo4j
