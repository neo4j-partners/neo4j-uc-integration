...
Fleet-wide overview combining Neo4j graph metrics with Delta sensor analytics in a single SQL statement. No Spark Connector needed, using pure SQL federation via Neo4j Federated JDBC UC Connection.

**Federation Method:** `remote_query()` (Neo4j Federated JDBC UC Connection) with CROSS JOINs
...
Neo4j: remote_query() via Neo4j Federated JDBC UC Connection | Delta: sensor_readings + sensors
================================================================================
+------------------------+---------------+-------------+--------------------------+---------------+-----------------+-----------------+----------------+--------------+
...
`remote_query()` returns a single-row table per call, making CROSS JOINs natural for combining multiple aggregate metrics. This avoids the GROUP BY limitation of Neo4j Federated JDBC UC Connection while keeping the query entirely in SQL.
