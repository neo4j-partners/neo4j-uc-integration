...
NOTE: The Neo4j JDBC driver requires additional memory for class loading in the Databricks SafeSpark sandbox. Without the correct Spark configuration (`spark.databricks.safespark.jdbcSandbox.jvm.maxMetaspace.mib`, etc.), the sandbox JVM will crash with "Connection was closed before the operation completed." See the full setup details in the https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[Neo4j UC JDBC Guide].
...
The SQL-to-Cypher translator now supports the following patterns that were previously unavailable:
...
|LIMIT and OFFSET
|`... GROUP BY name HAVING count(*) > 5 ORDER BY name LIMIT 10 OFFSET 5`

|WHERE + GROUP BY combinations
|`SELECT count(*) FROM People WHERE age > 18 GROUP BY name HAVING count(*) > 1`
...
The federated query examples in this guide use `remote_query()` for aggregate metrics (including GROUP BY and HAVING) and the Spark Connector for row-level data, combining both methods where needed.
...
2. Neo4j UC JDBC connection configured (see https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[setup guide])
3. Neo4j Spark Connector installed as cluster library (for queries 3-5)
4. `neo4j-uc-creds` secret scope configured

== References

* https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[Neo4j UC JDBC Setup Guide]
