...
NOTE: The Neo4j JDBC driver requires additional memory for class loading in the Databricks SafeSpark sandbox. Without the correct Spark configuration (`spark.databricks.safespark.jdbcSandbox.jvm.maxMetaspace.mib`, etc.), the sandbox JVM will crash with "Connection was closed before the operation completed." See the full setup details in the https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[Neo4j Federated JDBC UC Connection Setup Guide].
...
|`remote_query()`
|Pure SQL, no cluster library, UC governed, supports GROUP BY/HAVING/ORDER BY
|Non-aggregate SELECT not supported
...
The Neo4j Federated JDBC UC Connection supports aggregate queries, GROUP BY, HAVING, ORDER BY, DISTINCT, LIMIT/OFFSET, WHERE filters, NATURAL JOIN graph traversals, and their full combinations. Non-aggregate SELECT and relationship property aggregation are not supported — use the Neo4j Spark Connector for those.
...
The capabilities in this guide are possible today because of the custom Neo4j JDBC connector. But the connector is not yet an official Databricks federated data source, which limits what's available: Genie cannot query Neo4j directly, AI/BI dashboards cannot connect without materializing data into Delta first, and catalog-level discovery and column-level lineage require workarounds.
...
2. Neo4j Federated JDBC UC Connection configured (see https://github.com/neo4j-field/neo4j-uc-integration/blob/main/neo4j_uc_jdbc_guide.md[setup guide])
