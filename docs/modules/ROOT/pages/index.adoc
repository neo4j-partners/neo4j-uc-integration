...
This works because the Neo4j graph data (maintenance events, flights, airports) has been materialized as managed Delta tables via the UC JDBC connection. Genie sees all 8 tables as regular UC tables and generates federated SQL transparently. A set of Genie instructions teaches the LLM the domain model, JOIN patterns, and cross-source relationships. See the https://github.com/neo4j-partners/neo4j-uc-integration/blob/main/FEDERATED_AGENTS.md[Federated Agents guide] for the full architecture, materialization approach, agent integration patterns, and implementation checklist.
